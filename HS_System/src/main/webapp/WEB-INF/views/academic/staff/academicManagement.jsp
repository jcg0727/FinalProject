			...
			<div class="card card-primary card-outline card-outline-tabs">
				<div class="card-header p-0 border-bottom-0">
					<ul class="nav nav-tabs" id="custom-tabs-three-tab" role="tablist">
						<li class="nav-item"><a class="nav-link" id="custom-tabs-three-manage-tab" data-toggle="pill" href="#custom-tabs-three-manage" role="tab" aria-controls="custom-tabs-three-leave" aria-selected="false">휴학/복학/자퇴</a></li>
						<li class="nav-item"><a class="nav-link" id="custom-tabs-three-pural-tab" data-toggle="pill" href="#custom-tabs-three-pural" role="tab" aria-controls="custom-tabs-three-return" aria-selected="false" onclick="pluralMajorIn(); pluralMajorOut();">복수전공</a></li>
						<li class="nav-item"><a class="nav-link" id="custom-tabs-three-transfer-tab" data-toggle="pill" href="#custom-tabs-three-transfer" role="tab" aria-controls="custom-tabs-three-drop" aria-selected="false" onclick="transferInList(); transferOutList();">전입/전출</a></li>
					</ul>
				</div>
				<div class="card-body">
...
window.onload=function(){
	...
	$('#transferIn-table-body').on('click', '.counsel-set', function(e){
		...
		register_date = $(this).attr('register_date');        
	   	OpenWindow('<%=request.getContextPath()%>/counsel/assignment?dept_nm='+dept_nm+'&student_no='+student_no+'&register_date='+register_date,'상담신청','1300','800');
        e.stopPropagation();
   });
   ...
   $('#transferIn-table-body').on('click', '.advisor-set', function(e){
         ...
         OpenWindow('<%=request.getContextPath()%>/academic/admission/advisorList?dept_nm='+dept_nm+'&student_no='+student_no+'&professorYN=Y','지도교수배정','640','670');
         e.stopPropagation();
   })
   ...
   $('#transferIn-table-body').on('click','.transferIn-body', function(e){
	   ...
	   var register_date = $(this).attr('register_date');
		OpenWindow('<%=request.getContextPath()%>/academic/transfer/detail?student_no='+student_no+'&register_date='+register_date, '전입신청내역', '800', '700');
   });
