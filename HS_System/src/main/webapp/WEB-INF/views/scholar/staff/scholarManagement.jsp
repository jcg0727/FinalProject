      ...
      <div class="card-header">
         <div class="form-group form-group-sm row">
            <select class="form-control col-2" id="semester-select" name="lecture_semester" style="">
               <option value="" ${cri.lecture_semester eq '' ? 'selected' :'' } >학기 선택</option>
               <option value="202101" ${cri.lecture_semester eq '202101' ? 'selected':''}>21년도 1학기</option>
               <option value="202002" ${cri.lecture_semester eq '202002' ? 'selected':''}>20년도 2학기</option>
               <option value="202001" ${cri.lecture_semester eq '202001' ? 'selected':''}>20년도 1학기</option>
            </select>
            <div class="input-group input-group-mb-3 col-4" style="width: 250px;">
               <input type="text" id="scholar-dept-search" class="form-control float-right select-dept" id= "searchTypeDept" placeholder="학과명을 입력하세요" value="${cri.dept_nm }" <c:if test="${loginUser.authority_cd eq 'ROLE_STAFF' }"> readonly="readonly"</c:if> />
...
<%--                onclick="<c:if test="${loginUser.authority_cd eq 'ROLE_STAFF' }">scholarList();unopenedscholarList();registscholarList();</c:if><c:if test="${loginUser.authority_cd eq 'ROLE_SCHOLAR' }">list_goD(); </c:if>">검색</button>--%>
               onclick="<c:if test="${loginUser.authority_cd eq 'ROLE_STAFF' }">scholarList();unopenedscholarList();registscholarList();</c:if><c:if test="${loginUser.authority_cd eq 'ROLE_SCHOLAR' }">list_goScholar(); </c:if>">검색</button>
          </div>
         </div>
      </div>
      <div class="card-body">
