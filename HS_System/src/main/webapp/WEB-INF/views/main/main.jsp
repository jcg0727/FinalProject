            ...
            eventSources : [
                  {
                     ...
                     className : '수시모집' ,
                     color : '#97c783',  //rgb,#ffffff 등의 형식으로 할 수 있어요.
                  //textColor: 'black' 
                  },

                  {
                     ...
                     className : '대한민국의 공휴일',
                     color : '#2A5F9E', //rgb,#ffffff 등의 형식으로 할 수 있어요.
  ...
  <div class="card">
     <div class="card-header with-border" style="display: flex; height: 50px; padding-top: 0px;">
        <div id="keyword" class="">
          <h5 style="width: 100px; padding-top: 11px; font-weight: bold;">공지사항</h5>     
        </div>   
         <div class="input-group row " style="position: unset;padding-left: 550px;padding-bottom: 7px;font-size: 14px;padding-top: 7px;">
            ...
            <select class="form-control col-md-3" name="searchType2" id="searchType2" style="font-size: 14px;" hidden="true" >
              <option value=""  ${cri.searchType2 eq '' ? 'selected':''}>대상</option>
              <option value="학생"  ${cri.searchType2 eq '학생' ? 'selected':''}>학생</option>
              <option value="교수"  ${cri.searchType2 eq '교수' ? 'selected':''}>교수</option>
              <option value="교직원"  ${cri.searchType2 eq '교직원' ? 'selected':''}>교직원</option>
           </select>
           ...
           <!-- keyword -->
               <input  class="form-control" type="text" name="keyword" 
                       placeholder="검색어를 입력하세요." value="${cri.keyword }" style="font-size: 14px;height: 35px;"/>
...
<div class="card card-primary card-outline card-outline-tabs">
        <div class="card-header p-0 border-bottom-0">
          <ul class="nav nav-tabs" id="custom-tabs-three-tab" role="tablist" style="font-size: 15px;">
            <li class="nav-item">
              <a class="nav-link ${target eq '' ? 'active':'' }" id="공통" data-toggle="pill" onclick="Select2('');"  role="tab" aria-controls="custom-tabs-three-profile" aria-selected="false">공통</a>
            </li>
            <li class="nav-item">
              <a class="nav-link ${target eq '학생' ? 'active':'' }" id="학생" data-toggle="pill" onclick="Select2('학생');" role="tab" aria-controls="custom-tabs-three-profile" aria-selected="false">학생</a>
            </li>
            <li class="nav-item">
              <a class="nav-link ${target eq '교수' ? 'active':'' }" id="교수" data-toggle="pill" href="" onclick="Select2('교수');"  role="tab" aria-controls="custom-tabs-three-messages" aria-selected="false">교수</a>
            </li>
            <li class="nav-item">
              <a class="nav-link ${target eq '교직원' ? 'active':'' }" id="교직원" data-toggle="pill" href="" onclick="Select2('교직원');" role="tab" aria-controls="custom-tabs-three-settings" aria-selected="false">교직원</a>
            </li>
          </ul>
        </div>
         ...
         <table class="table table-hover table-sm" style="font-size: 15px;">
            ...
            <tr style="text-align: center;">
              ...
              <c:forEach items="${YList }" var="y">
                   <tr onclick="OpenWindow('detail.do?notice_no=${y.notice_no}&from=list','상세보기',950,630);" style="text-align: center; font-size: 14px; font-weight: bold;">
              ...
              <c:forEach items="${noticeList }" var="n">
                   <tr onclick="OpenWindow('detail.do?notice_no=${n.notice_no}&from=list','상세보기',950,630);" style="text-align: center; font-size: 14px;">
                   <td>${n.num }</td>
...
<nav aria-label="Navigation" style="display: flex;">
   <ul class="pagination justify-content-center m-0" style="padding-left: 360px;">
      ...
      <button type="button" class="btn btnBlue btn-sm text-white" style="background-color:#2A5F9E; margin-left: 300px;" onclick="OpenWindow('registForm.do','글등록',950,660);">등록</button>
     </c:if>
</nav>                              
   </div>
   ...
   <div class="card-body" style="padding-bottom: 0px;padding-left: 10px;padding-right: 10px;padding-top: 0px;">
      <div class="card">
         <div class="card-header" style="height: 50px;">
            <h5 class="" style="font-weight: bold;">학사일정</h5>
         </div>
