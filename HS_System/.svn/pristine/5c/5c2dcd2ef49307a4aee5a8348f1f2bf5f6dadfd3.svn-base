...
<div class="" style="width:80%; height:90%; margin-left: 10%; margin-top:10%; border-width: thin; border-color: gray; background-position: bottom; border-style: solid; background-image: url('<%=request.getContextPath() %>/resources/logo3.png');" id="back">
    ...
    <div class="row">
      <div class="col-12">
        <h1 style="text-align: center;margin-top: 10%;">등록금고지서</h1>
      </div>
    </div>
    <br>
      ...
      <div style="margin-left: 10%; margin-top: 10%; margin-bottom: 8%">
         <c:forEach items="${tuitionDetailList }" var="list">
		       ...
		       <c:set var="semester" value = "${list.semester }"/>
		        ...
		        <h5>성&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;명&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; :&nbsp;&nbsp;&nbsp;&nbsp; ${list.name }</h5>
		        <h5>생&nbsp;년&nbsp;월&nbsp;일&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; : &nbsp;&nbsp;&nbsp;&nbsp;19${fn:substring(TextValue,0,2)}년${fn:substring(TextValue,2,4)}월${fn:substring(TextValue,4,6)}일 </h5>
		      	
			 		 <button type="button" class="btn btnRed btn-sm text-white" onclick="requestPay();" style="margin-left:489px; margin-bottom:10px; font-weight: bold">등록금 납부</button>
		            ...
		            <tr>
		                ...
		                <tr>
		                  ...
		                  <td>
			   		    ...
			   		    <form id="tuitionPay">
							<input type='hidden' name="stuNo" value="${list.student_no}" />
						    <input type='hidden' name="Sem" value="${list.semester}"/>
							<input type='hidden' name="Amount" value="${list.payment_amount}" />
						</form>
...
<script>
function payMentTuition(){

	var student_no = $('input[name="stuNo"]').val();
	var semester   = $('input[name="Sem"]').val();
	...
	$.ajax({
		...
		url : '<%=request.getContextPath()%>/tuition/payUrl',
 		data : {"student_no":student_no, "semester":semester, "payment_amount":payment_amount},
 		//dataType : "json",
...
/* 	console.log(student_no);
	...
	IMP.request_pay({
	    ...
	    name : "등록금납부",
	    amount : payment_amount, //판매 가격
	    buyer_name : student_no
	}, function(rsp) {
	    if ( rsp.success ) {
	            var msg = "결제 진행중입니다.";
	  			/* regist(student_no, payment_amount, semester);   */
	  			
	        	$.ajax({
	        		url : '<%=request.getContextPath()%>/tuition/payResult',
	        		data : {"student_no":student_no, "payment_amount":payment_amount, "semester":semester},
	        		//dataType : 'json',
	        		success: function(data){
	        			alert("결제가 완료되었습니다.");
	        			window.close();
...
function regist(student_no, payment_amount, semester){
	...
	$.ajax({
		url : '<%=request.getContextPath()%>/tuition/payResult',
		data : {"class_no":class_no, "payment_amount":payment_amount, "semester":semester},
		dataType : 'json',
		success: function(data){
			alert("결제가 완료되었습니다.");
            opener.parent.location.reload(); 
