...
<script>
function ChangeAdv(pro_no,student_no,professorYN){
   console.log(professorYN);
   if(professorYN=="N"){
      confirm("지도교수를 등록하시겠습니까?");      
      $.ajax({
         url : '<%=request.getContextPath()%>/academic/admission/freshmen/insertAdvisor',
         data : {"pro_no" : pro_no, 
   ...
   $.ajax({
      url : '<%=request.getContextPath()%>/academic/admission/freshmen/changeAdvisor',
                  data : {
                     "pro_no" : pro_no,
                     "student_no" : student_no
                  },
      ...
      <div class="card">
         <div class="card-header with-border" style="border-bottom-width: 3px; border-bottom-color: #2A5F9E">
            <div id="keyword" class="">
               <h5 style="font-weight: bold;">${dept_nm }&nbsp;교수목록</h5>
            </div>
         </div>
         <div class="card-body" style="text-align: center; padding-bottom: 0; padding-top: 7px;">
            <div class="row">
               <div class="input-group " style="position: unset; padding-bottom: 7px; padding-left: 224px; font-size: 14px; padding-right: 8px;">
                  ...
                  <span class="input-group-append">
                     <button class="btn btnBlue text-white" style="background-color:#2A5F9E;" type="button" id="searchBtn" data-card-widget="search" onclick="list_go(1);">
                  ...
                  <table class="table table-hover table-sm" style="font-size: 15px;">
                     ...
                     <c:forEach items="${proList }" var="list">
                        <tr>
                           ...
                           <td>${list.perStu }</td>
                           <td>
                              <button type="button" class="btn btn-xs btnBlue text-white" style="background-color:#2A5F9E;" id="changeAdvisor" onclick="ChangeAdv('${list.pro_no}','${student_no }','${professorYN }');" style="height: 21px;">선택</button>
                           </td>
                        </tr>
                     </c:forEach>
            ...
            <div>
               ...
               <div style="padding-top: 10px; padding-bottom: 10px; float: none;">
                  <button type="button" class="btn btn-sm btnGray text-white" id="cancelBtn" onclick="CloseWindow();">닫기</button>
               </div>
            </div>
         </div>
      </div>
