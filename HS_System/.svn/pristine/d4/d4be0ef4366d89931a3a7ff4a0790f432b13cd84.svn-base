...
<c:set var="stuStatusList" value="${dataMap.stuStatusList }" />
...
<div class="card">
  <div class="card-header with-border staffStatus-card" style="border-bottom-width: 3px; border-bottom-color: #2A5F9E">
   ...
   <div class="card-body table-responsive p-0" style="height:270px;">
      ...
      <c:forEach items="${stuStatusList }" var="list" >
         <tr onclick="DetailStuSta('${list.student_no }','<fmt:formatDate value="${list.register_date }" pattern="yyyy/MM/dd"/>', '${list.r_status_cd}' );" >
             <td>${list.num }</td>
...
<div class="card">
  <div class="card-header with-border" style="border-bottom-width: 3px; border-bottom-color: #2A5F9E"><h5 style="font-weight: bold;">학적변경 상세보기</h5></div>
   <div class="card-body" style="text-align:left; padding-bottom: 0; padding-top: 7px;">
    ...
    <button type="button" id="confirm_s" class="btn btnBlue btn-sm text-white" onclick="confirm_s('CCDF006');" style="display: none;">승인</button>
    <button type="button" id="confirm_b" class="btn btnRed btn-sm text-white" onclick="confirm_b('CCDF007');" style="display: none;">반려</button>
   </div>
</div> <!-- card-body -->

<script>
function DetailStuSta(student_no, register_date, r_status_cd) {
	...
	$.ajax({
		...
		success: function(data){
			$(data).each(function(){
				...
				if(r_status_cd == 'CCDF005' || 'CCDF006' || 'CCDF007'){
				document.getElementById("confirm_s").style.display = "";
				document.getElementById("confirm_b").style.display = "";
				console.log("hi");
				}else{
